...
- [ ] Read `notion_client.py` (307 lines) — understand the full public API
  surface: `NotionClient`, block builders (`heading_2`, `paragraph`, `table`,
  `callout`, `divider`, `bulleted_list`, `toggle`)
- [ ] Read `notion_writer.py` — understand the orchestrator flow: parent page
  creation, sub-page creation order, state file write, subprocess calls
- [ ] Read `notion_bible.py` — understand how Bible data maps to Notion blocks
- [ ] Read `notion_weather.py` — understand weather data → Notion blocks
- [ ] Read `notion_finance.py` — understand finance data → Notion blocks
  (tables, callouts, toggles)
- [ ] Read `notion_todoist.py` — understand Todoist tasks → Notion blocks
- [ ] Read `notion_birthdays.py` — understand birthday data → Notion blocks
- [ ] Read all 7 test files in `tests/` to understand the mocking patterns
  (how `NotionClient` is patched, fixture structure, assertion style)
- [ ] Read `setup-guide.md` (project root, NOT planning_docs/) — understand
  the current Notion setup steps (Step 4), ChatGPT integration (Step 10),
  and deployment steps (Step 13c)
- [ ] Read `implementation_plan_anytype.md` — fully understand the design
  decisions and file mapping
- [ ] **Add Anytype aliases to `fm_config.py`** (lines 118-119). Add these
...
- [ ] Run `./scripts/check.sh` — must pass (both old and new names coexist)
...
- [ ] Create `tests/test_anytype_client.py`
- [ ] Test `AnytypeClient.__init__` — verify `httpx.Client` is created with
  correct base URL, headers (`Authorization`, `Anytype-Version`, `Content-Type`)
- [ ] Test `AnytypeClient.list_spaces()` — mock `GET /v1/spaces`, verify
  return value and error handling
- [ ] Test `AnytypeClient.create_object()` — mock `POST /v1/spaces/{sid}/objects`,
  verify payload shape: `name`, `type_key`, `icon`, `body`
- [ ] Test `AnytypeClient.update_object()` — mock `PATCH /v1/spaces/{sid}/objects/{oid}`,
  verify partial update handling (body only, name only, both)
- [ ] Test `AnytypeClient.get_object()` — mock `GET /v1/spaces/{sid}/objects/{oid}`
- [ ] Test `AnytypeClient.search_objects()` — mock `POST /v1/spaces/{sid}/search`,
  verify query/types/sort payload and pagination params
- [ ] Test `AnytypeClient.delete_object()` — mock `DELETE /v1/spaces/{sid}/objects/{oid}`
- [ ] Test all Markdown builders: `md_heading`, `md_paragraph`, `md_bullet`,
...
- [ ] Test HTTP error handling: `httpx.HTTPStatusError` for 401, 404, 429

### 1.2 Write the implementation (TDD Green)

- [ ] Create `anytype_client.py`
- [ ] Implement `AnytypeClient` class with all methods
- [ ] Implement all `md_*` Markdown builder functions (module-level)
- [ ] Ensure all tests pass: `pytest tests/test_anytype_client.py -v`

### 1.3 Refactor and verify

- [ ] Run `./scripts/check.sh` — all 6 gates must pass
- [ ] Verify no `Any` types in function signatures (except API response dicts)
- [ ] Verify Google-style docstrings on all public functions
...
- [ ] Create `tests/test_anytype_writer.py`
- [ ] Test parent object creation: verify `create_object` called with correct
  name pattern (`📋 Daily Brief — {date}`) and `type_key="page"`
- [ ] Test sub-page creation: verify each domain module is called and its
  content is passed to `create_object`
- [ ] Test state file write: verify `/tmp/daily_brief_state.json` is written
  with correct structure (`space_id`, `parent_object_id`, `sub_objects`)
- [ ] Test error handling: verify graceful failure when Anytype API is down
- [ ] Test environment variable loading: `ANYTYPE_API_KEY`, `ANYTYPE_SPACE_ID`,
  `ANYTYPE_API_URL`

### 2.2 Write the implementation (TDD Green)

- [ ] Create `anytype_writer.py`
- [ ] Implement the orchestrator flow:
...
- [ ] Ensure all tests pass: `pytest tests/test_anytype_writer.py -v`
...
- [ ] Create `tests/test_anytype_bible.py` (adapt from `test_notion_bible.py`)
...
- [ ] Create `anytype_bible.py` (adapt from `notion_bible.py`)
...
- [ ] Run `pytest tests/test_anytype_bible.py -v`
- [ ] Run `./scripts/check.sh`

### 3.2 `anytype_weather.py`

- [ ] Create `tests/test_anytype_weather.py` (adapt assertions to Markdown strings)
- [ ] Create `anytype_weather.py` (returns Markdown string, not block list)
- [ ] Run `pytest tests/test_anytype_weather.py -v`
- [ ] Run `./scripts/check.sh`
...
- [ ] Create `tests/test_anytype_finance.py`
...
- [ ] Run `pytest tests/test_anytype_finance.py -v`
- [ ] Run `./scripts/check.sh`

### 3.4 `anytype_todoist.py`

- [ ] Create `tests/test_anytype_todoist.py` (adapt assertions to Markdown strings)
- [ ] Create `anytype_todoist.py` (returns Markdown string, not block list)
- [ ] Run `pytest tests/test_anytype_todoist.py -v`
- [ ] Run `./scripts/check.sh`

### 3.5 `anytype_birthdays.py`

- [ ] Create `tests/test_anytype_birthdays.py` (adapt assertions to Markdown strings)
- [ ] Create `anytype_birthdays.py` (returns Markdown string, not block list)
- [ ] Run `pytest tests/test_anytype_birthdays.py -v`
- [ ] Run `./scripts/check.sh`
...
- [ ] Rewrite Step 4 in `setup-guide.md` (project root, NOT planning_docs/):
...
- [ ] Add new step between Steps 3 and 4 (renumber as needed):
...
- [ ] Search for ALL remaining "Notion" / "notion" / "NOTION" references in
  `setup-guide.md` (project root!) — fix every one
- [ ] Verify the document flows logically end-to-end
...
- [ ] Delete `notion_client.py`
- [ ] Delete `notion_writer.py`
- [ ] Delete `notion_bible.py`
- [ ] Delete `notion_weather.py`
- [ ] Delete `notion_finance.py`
- [ ] Delete `notion_todoist.py`
- [ ] Delete `notion_birthdays.py`
- [ ] Delete `tests/test_notion_client.py`
- [ ] Delete `tests/test_notion_writer.py`
- [ ] Delete `tests/test_notion_bible.py`
- [ ] Delete `tests/test_notion_weather.py`
- [ ] Delete `tests/test_notion_finance.py`
- [ ] Delete `tests/test_notion_todoist.py`
- [ ] Delete `tests/test_notion_birthdays.py`
- [ ] Run `./scripts/check.sh` — all gates must pass with ONLY Anytype files
...
- [ ] Verify no `notion_*` files exist:
  ```bash
  ls notion_*.py 2>&1
  ls tests/test_notion_*.py 2>&1
  ```
- [ ] Verify unchanged test files still pass:
  ```bash
  pytest tests/test_bible_reading.py tests/test_weather_fetch.py \
    tests/test_financial_monitor.py -v
  ```
- [ ] Final full gate pass: `./scripts/check.sh` — all 6 gates, 0 failures

---

## Phase 7 — Cleanup and `pyproject.toml`

- [ ] **Remove old NOTION_* aliases from `fm_config.py`** — delete the lines:
  ```python
...
- [ ] Update `pyproject.toml` if any per-file-ignores reference `notion_*` files
- [ ] Clean up `__pycache__` directories:
  ```bash
  find . -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null
  ```
- [ ] Final `./scripts/check.sh` run
- [ ] Summarize all files modified/created/deleted in a final message
- [ ] **STOP** — do NOT commit. Await explicit user approval.
