...
from vigil.financial.fetchers import parse_checklist
from vigil.financial.monitor import (
    build_daily_summary,
    main,
)
...
class TestSnapTradeFailureDegrades:
    def test_brokerage_none_partial_data(self) -> None:
        with (
            patch(
                "vigil.financial.monitor.fetch_brokerage_data",
                return_value=None,
            ),
            patch(
                "vigil.financial.monitor.fetch_bank_data",
                return_value=None,
            ),
            patch(
                "vigil.financial.monitor.fetch_market_data",
                return_value=None,
            ),
            patch(
                "vigil.financial.monitor.fetch_recession_indicators",
                return_value=None,
            ),
            patch(
                "vigil.financial.monitor.fetch_hyperscaler_capex",
                return_value=None,
            ),
            patch(
                "vigil.financial.monitor.SNAPTRADE_CONSUMER_KEY",
                "test-key",
            ),
        ):
            main()
...
class TestOfxFailureDegrades:
    def test_bank_none_partial_data(
        ...
        with (
            ...
            patch(
                "vigil.financial.monitor.OFX_BANKS_CONFIG",
                [{"name": "test"}],
            ),
        ):
            main()
...
class TestFredFailureDegrades:
    def test_fred_none_with_key(
        ...
        with (
            ...
            patch("vigil.financial.monitor.FRED_API_KEY", "test-key"),
        ):
            main()
...
class TestMainStatusCritical:
    def test_sahm_rule_sets_critical_status(
        ...
        with (
            patch("vigil.financial.monitor.fetch_brokerage_data", return_value=None),
            patch("vigil.financial.monitor.fetch_bank_data", return_value=None),
            patch("vigil.financial.monitor.fetch_market_data", return_value=None),
            patch(
                "vigil.financial.monitor.fetch_recession_indicators",
                return_value={
                    "unemployment_rate": {
            ...
            patch("vigil.financial.monitor.fetch_hyperscaler_capex", return_value=None),
        ):
            main()
...
class TestMainStatusPartialDataWithHighAlerts:
    def test_partial_data_preserved_over_high(
        ...
        with (
            ...
            patch("vigil.financial.monitor.SNAPTRADE_CONSUMER_KEY", "test-key"),
        ):
            main()
...
class TestSafeJsonInvalidInput:
    def test_invalid_json_exits(self) -> None:
        from vigil.config import safe_json
...
class TestSafeIntInvalidInput:
    def test_non_numeric_exits(self) -> None:
        from vigil.config import safe_int
