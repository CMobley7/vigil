...
class TestFetchReturnsValidJson:
    def test_output_has_required_keys(self) -> None:
        ...
        with patch("vigil.weather.httpx.get", return_value=mock_resp):
...
class TestEnvVarDefaults:
    def test_missing_weather_lat_uses_default(self) -> None:
        import vigil.weather as weather_module

        assert weather_module.LATITUDE == pytest.approx(35.2271)
        assert weather_module.LONGITUDE == pytest.approx(-80.8431)


class TestSafeFloatInvalidInput:
    def test_non_numeric_exits(self) -> None:
        from vigil.config import safe_float
