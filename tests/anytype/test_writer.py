...
class TestMain:
    ...
    @patch("vigil.anytype.writer.AnytypeClient")
    @patch(
        "vigil.anytype.writer.extract_today_reading",
        return_value={
            "morning_devotional": {"day": "Day 1", "text": "T"},
            "bible_reading": {
                "reference": "Gen 1",
    ...
    @patch("vigil.anytype.writer.fetch_weather", side_effect=RuntimeError("API down"))
    @patch("vigil.anytype.writer.fetch_todoist_tasks", return_value=[])
    @patch("vigil.anytype.writer.check_birthdays_today", return_value=[])
    @patch(
        "vigil.anytype.writer.subprocess.check_output",
    ...
    @patch(
        "vigil.anytype.writer.fetch_weather",
        return_value={"today_hourly": [], "ten_day_forecast": []},
    )
    @patch(
        "vigil.anytype.writer.fetch_todoist_tasks",
    ...
    @patch("vigil.anytype.writer.fetch_todoist_tasks", return_value=[])
    @patch(
        "vigil.anytype.writer.check_birthdays_today",
